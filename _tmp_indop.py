...
class IndependentOperator(OpenMCOperator):
    ...
    def estimate_k_inf(self, nu_fission=None):
        """Estimate :math:`k_{\\infty}` for the current material
        compositions and microscopic cross sections, following the
        formulation proposed by Cetnar et al. (Ann. Nucl. Energy, 2014;
        https://doi.org/10.1016/j.anucene.2014.04.021).

        The estimate is

        .. math::

            k_{\\infty} = \\frac{\\sum_i N_i \\sum_j \\nu_j
                          \\sum_g \\phi_g \\sigma_{i,j,g}}
                         {\\sum_i N_i \\sum_j
                          \\sum_g \\phi_g \\sigma_{i,j,g}}

        where the sum over :math:`j` runs over all reactions in the
        :class:`~openmc.deplete.MicroXS` data. The neutron multiplicity
        :math:`\\nu_j` is:

        * 0 for non-multiplicative reactions (capture, charged-particle, etc.)
        * :math:`x` for ``(n,xn)`` reactions (parsed from the reaction name)
        * a per-nuclide value for ``fission`` taken from ``nu_fission`` if
          provided, otherwise from a small built-in default table for common
          thermal-fission isotopes (U235, U238, Pu239, Pu241).

        This is a transport-independent estimate, useful for sanity-checking
        depletion inputs against tools such as ORIGEN-S. It is **not** a
        substitute for a full transport solve and will be inaccurate in
        spectra where the default :math:`\\bar{\\nu}` values are poor
        approximations (e.g., fast reactors with significant high-energy
        fission).
        ...
        Parameters
        ----------
        nu_fission : dict of str to float, optional
            Mapping of nuclide name to :math:`\\bar{\\nu}` (average neutrons
            per fission) used when the ``fission`` reaction is encountered.
            If a nuclide is missing from this mapping, the built-in defaults
            are consulted; if still missing, a :class:`ValueError` is raised.

        Returns
        -------
        k_inf : float
            Estimated :math:`k_{\\infty}` for the current operator state.

        """
        # Default nu_bar values for common thermal-spectrum fissioning
        # nuclides. These are coarse averages intended for sanity checking;
        # users with energy-resolved data should pass nu_fission explicitly.
        default_nu = {
            'U235': 2.43,
            'U238': 2.50,   # fast-fission average
            'Pu239': 2.88,
            'Pu241': 2.93,
            'U233': 2.49,
            'Pu240': 2.80,
        ...
        if nu_fission is not None:
            default_nu.update(nu_fission)

        def _nu_for(rx, nuc):
            """Return neutron multiplicity nu_j for one (nuc, rx) pair."""
            if rx == 'fission':
                if nuc in default_nu:
                    return default_nu[nuc]
                raise ValueError(
                    f"nu_bar for fission of {nuc} is unknown. "
                    "Pass it via the nu_fission argument."
                )
            # (n,xn) reactions: parse the integer from the label,
            # e.g. (n,2n) -> 2
            if rx.startswith('(n,') and rx.endswith('n)'):
                digits = rx[3:-2]
                if digits.isdigit():
                    return int(digits)
            # All other reactions (capture, charged-particle, etc.) do not
            # multiply neutrons.
            return 0
        ...
        for mat, flux, micro in zip(self.materials, self.fluxes, self.cross_sections):
            if mat.volume is None:
                raise ValueError(
                    f"Material {mat.name!r} has no volume set; cannot "
                    "estimate k_inf."
                )
            # Atom densities are atom/b-cm. The volume cancels in the
            # k_inf ratio, so we only need the relative concentrations.
            density = mat.get_nuclide_atom_densities()
            flux_arr = np.asarray(flux, dtype=float)
            if flux_arr.ndim == 0:
                flux_arr = np.atleast_1d(flux_arr)

            for nuc, N_i in density.items():
                if nuc not in micro._index_nuc:
                    continue
                i_nuc = micro._index_nuc[nuc]
                for rx in micro.reactions:
                    i_rx = micro._index_rx[rx]
                    sigma_ig = micro.data[i_nuc, i_rx, :]
                    if not np.any(sigma_ig):
                        continue
                    nu_j = _nu_for(rx, nuc)
                    weighted = float(np.sum(flux_arr * sigma_ig))
                    if nu_j != 0:
                        numerator += N_i * nu_j * weighted
                    denominator += N_i * weighted

        if denominator == 0.0:
            raise ValueError(
                "Denominator of k_inf estimate is zero; no (nuclide, "
                "reaction) pairs with non-zero cross sections were found."
            )
